...
def _(df_final, mo):
    ...
    # 3. A Dropdown for the Sector Box Plot tab
    ...
    # 4. LLM Prompt Builder inputs (Week 8)
    ...
    context_input = mo.ui.text_area(
        ...
        label="Context (grounding data)",
        rows=2,
        )
    temperature_slider = mo.ui.slider(
        start=0.0, stop=1.0, step=0.1, value=0.2,
        label="Temperature",
        )

    return (all_sectors, cap_slider, context_input, persona_input,
            sector_box_select, sector_dropdown, task_input, temperature_slider)
...
@app.cell
def _(count, cv_download_btn, df_final, filtered_portfolio, mo, pd, px,
       sector_box_select, context_input, persona_input, task_input, temperature_slider):
    # Create all plots and tab content based on filtered data
    import numpy as np

    # Plot 1: Credit Risk Scatter 
    ...
    if not df_regline.empty:
        x_r = df_regline['Z_Score_lag'].astype(float)
        y_r = df_regline['Debt_Cost_Percent'].astype(float)
        ...
        line_trace.update(line=dict(width=1.5, color='black', dash='dot'))
    ...
    # Self-exploration: auto summary stats table
    summary_stats = (
    ...
    _prompt = (
        ...
        f"You are a {persona_input.value}. Your task is to {task_input.value}.\n\n"
        "## INSTRUCTIONS\n"
    ...
    return (chart_box, chart_element, chart_esg, fig_travel,
            prompt_preview, summary_stats)


# Cell 9: Tab Layout 
@app.cell
def _(cap_slider, chart_box, chart_element, chart_esg, context_input,
       cv_download_btn, fig_travel, mo, persona_input, prompt_preview,
       sector_box_select, sector_dropdown, summary_stats,
       task_input, temperature_slider, avg_cost_filtered, count):
...
**Languages:** English (Fluent) · Urdu (Fluent)
    ...
    tab_data_content = mo.vstack([
        ...
        mo.callout(mo.md(
            ...
            "Extended the template with a live **OLS regression line** via `numpy.polyfit`."
            ), kind="info"),
...
- Reactive prompt builder (Week 4 reactivity applied to LLMs) — **self-exploration**
        """),
        ])
